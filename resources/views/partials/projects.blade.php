...
<section id="projects" class="max-w-5xl mx-auto px-6 py-16">
    ...
    <h2 class="font-serif text-4xl md:text-5xl font-black mb-10">Projects</h2>
            ...
            <div class="md:w-1/2 flex flex-col justify-center">
                ...
                <div class="flex gap-4 font-mono text-xs uppercase tracking-widest">
                    @if($featuredProject->live_url)
                        <a href="{{ $featuredProject->live_url }}" target="_blank" class="border-b border-[#8db800] text-[#5a7a00] hover:text-black">Live →</a>
                    @endif
                    @if($featuredProject->github_url)
                        <a href="{{ $featuredProject->github_url }}" target="_blank" class="border-b border-black hover:text-[#5a7a00]">Code →</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="border border-black p-5 flex flex-col">
            ...
            <img src="{{ asset('storage/' . $project->thumbnail_path) }}"
                 alt="{{ $project->title }}"
                 class="w-full h-40 object-cover border border-black mb-4">
            @endif

            <h3 class="font-serif text-xl font-bold mb-2">{{ $project->title }}</h3>
            <p class="text-sm mb-4 flex-grow">{{ $project->description }}</p>
            ...
            <div class="flex gap-4 font-mono text-xs uppercase tracking-widest mt-auto">
                @if($project->live_url)
                    <a href="{{ $project->live_url }}" target="_blank" class="border-b border-[#8db800] text-[#5a7a00] hover:text-black">Live →</a>
                @endif
                @if($project->github_url)
                    <a href="{{ $project->github_url }}" target="_blank" class="border-b border-black hover:text-[#5a7a00]">Code →</a>
                @endif
            </div>
        </div>
        @endforeach
